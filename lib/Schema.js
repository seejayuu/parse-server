...
var Schema = function () {
  ...
  _createClass(Schema, [{
    ...
    value: function reloadData() {
      ...
      return this._collection.getAllSchemas().then(function (allSchemas) {
        allSchemas.forEach(function (schema) {
          ...
          _this.data[schema.className] = parseFormatSchema;
    ...
    value: function addClassIfNotExists(className) {
      ...
      return this._collection.addSchema(className, fields, classLevelPermissions).then(function (res) {
    ...
    value: function updateClass(className, submittedFields, classLevelPermissions, database) {
      ...
      var validationError = this.validateSchemaData(className, newSchema, classLevelPermissions);
      if (validationError) {
    ...
    value: function validateField(className, fieldName, type, freeze) {
      ...
      return this.reloadData().then(function () {
        ...
        transform.transformKey(_this5, className, fieldName);
        ...
        if (type === 'GeoPoint') {
          ...
          for (var otherKey in _this5.data[className]) {
            if (_this5.data[className][otherKey].type === 'GeoPoint') {
              throw new Parse.Error(Parse.Error.INCORRECT_TYPE, 'there can only be one geopoint field in a class');
            }
          }
        }

        return _this5._collection.updateField(className, fieldName, type).then(function () {
          // The update succeeded. Reload the schema
    ...
    value: function deleteField(fieldName, className, database) {
      ...
      return this.reloadData().then(function () {
        ...
        return database.adaptiveCollection(className).then(function (collection) {
          return database.adapter.deleteFields(className, fieldNames, pointerFieldNames, database.collectionPrefix, collection);
        });
      });
    }
