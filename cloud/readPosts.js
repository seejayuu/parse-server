	...
	queryBlocks().then(function(blockList) {
		Parse.Promise.when(promises).then(function(results) {
			...
			try {
				...
				finalResults = _.filter(finalResults, function(post) { return typeof(post.get("from")) != "undefined" || _.filter(blockList, function (i) { return typeof(i.toPost.id) != 'undefined' && i.toPost.id == post.id }).length == 0 } );
